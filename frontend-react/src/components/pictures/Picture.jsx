...
const Picture = () => {
  ...
  const checkIfUserBoughtPicture = () => {
    const exists = user?.orders.some(
      (order) => order.picture.id === picture?.id
    );
    if (exists) {
      return (
        <button className="btn btn-primary" onClick={() => downloadPicture()}>
  ...
  const downloadPicture = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/download/picture/${picture?.id}`,
        {
          headers: {
          ...
          <div className="col-md-6 mx-auto">
            <div className="card">
              <div className="card-body">
                <div className="alert alert-info my-3">{error}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="col-md-6">
            <div className="card">
              <div className="card-header bg-white">
                <h4 className="text-center mt-2 mb-4">{picture?.title}</h4>
                    ...
                    <div className="d-flex flex-column mx-2">
                      ...
                      <span className="text-muted">
                        <i>
                          ...
                          {picture?.user.pictures.length > 1
                            ? "pictures"
                            : "picture"}
                        </i>
                      </span>
              ...
              <div className="card-body">
                  ...
                  {calculateReviewAverage() > 0 && (
                    <div className="d-flex align-items-center">
                      ...
                      <Rating
                        ...
                        size={32}
                      />
                      <span className="mx-1">{calculateReviewAverage()}</span>
                    </div>
                  )}
                </div>
                <div className="row my-3">
                  <div className="d-grid gap-2 col-10 mx-auto">
                    ...
                    <Reviews picture={picture} setLoading={setLoading} />
                  </div>
                </div>
              </div>
            </div>
